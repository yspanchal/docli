...
@click.option('--power-off', '-s', type=int, help='Power Off droplet for given droplet id', metavar='<3812352>')
@click.pass_context
def droplet_actions(ctx, disable_backups, reboot, shutdown, power_off):
	"""
	Droplet actions are tasks that can be executed on a Droplet.
	These can be things like rebooting, resizing, snapshotting, etc.
	"""

	if (not ctx.params['disable_backups'] and not ctx.params['reboot'] and not ctx.params['power_cycle'] and not ctx.params['shutdown'] and not ctx.params['power_off']):
